...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseErrorDto> handleNotFound(EntityNotFoundException ex){
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body(new ResponseErrorDto("Not Found", HttpStatus.NOT_FOUND.name(), 404, null));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ResponseErrorDto> handleException(Exception ex){
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .body(new ResponseErrorDto("Internal Server Error", HttpStatus.INTERNAL_SERVER_ERROR.name(), 500, null));
    }
}
